...
def init_db():
    ...
    try:
        ...
        try:
            ...
            supabase.table("pickup_requests").select("count", count="exact").execute()
        except Exception:
            print("Creating pickup_requests table...")
            # Create pickup_requests table using SQL
        ...
        for laundry_type in laundry_types:
            ...
            response = supabase.table("laundry_types").select("*").eq("id", laundry_type["id"]).execute()
        ...
        for payment_method in payment_methods:
            # Check if it already exists
            response = supabase.table("payment_methods").select("*").eq("id", payment_method["id"]).execute()
            if not response.data or len(response.data) == 0:
